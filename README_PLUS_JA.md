...
3. プロンプト欄で調べたいタグ付近にカーソルを置き、`Alt+Q` を押すと関連タグパネルが開きます。
4. 候補をクリックすると、元のタグの後ろに挿入され、その候補を起点に次の関連タグへ更新されます。
5. 関連タグパネルの `‹` / `›`、または `Alt+←` / `Alt+→` で連想検索の履歴を戻る/進むできます。

`Alt+Q only` モードでは、クリックだけでは関連タグ検索を行いません。
プロンプト欄にカーソルがある時は、軽いヒントとして `Alt+Qで関連タグ検索` が表示されます。
...
Plus 版では、関連タグパネル、`Alt+Q` 起動、日本語ラベル、機械翻訳補完はいずれも初期状態で有効です。
